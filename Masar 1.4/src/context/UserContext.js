...
// Use your machine's LAN IP so Expo app (device/emulator) can reach the backend.
// Metro shows exp://172.20.10.9:8082 — use that host with backend port 5001.
// NOTE: Backend server runs on port 5001. Update the host:port below if different.
export const API_URL = 'http://172.20.10.9:5001'; // <-- Backend server
  ...
  const createBooking = async ({ from, to, date, departureTime, arrivalTime, price, duration, route }) => {
    ...
    try {
      ...
      if (resp.data.success) {
        ...
        const entry = { from: b.from, to: b.to, cost: b.price, mode: 'Train', time: new Date(b.createdAt).toLocaleString(), duration: b.duration ? `${b.duration} min` : '' };
  ...
  const recordScan = async ({ amount = 12.5, station = 'Metro Station' } = {}) => {
    ...
    try {
      ...
      if (resp.data.success) {
        ...
        setUser(u => ({ ...u, balance: resp.data.balance }));
        const b = resp.data.booking;
        const entry = { from: b.from, to: b.to, cost: b.price, mode: 'Scan', time: new Date(b.createdAt).toLocaleString(), duration: '0 min' };
        setRecentTrips(prev => [entry, ...prev].slice(0, 100));
        return { success: true, booking: resp.data.booking };
      }
