...
export default function Profile() {
  ...
  const { appTheme, appLanguage, user, isLoggedIn, logout } = useContext(UserContext);
        ...
        {isLoggedIn && user ? (
          ...
          <>
            <View style={{ backgroundColor: T.primaryAlt, borderRadius: 24, padding: 18, marginBottom: 24 }}>
              ...
              <Text style={{ color: 'rgba(255,255,255,0.8)', textAlign: isRTL ? 'right' : 'left' }}>
                {tr('memberSince')} {new Date(user.createdAt).toLocaleDateString()}
              </Text>
            </View>

            {/* User Info */}
            <Text style={{ color: T.text, fontWeight: '800', fontSize: 16, marginBottom: 12, textAlign: isRTL ? 'right' : 'left' }}>
              {tr('personalInfo')}
            </Text>
            ...
            <View style={{
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: T.subtext, fontSize: 12, marginBottom: 4, textAlign: isRTL ? 'right' : 'left' }}>{tr('email')}</Text>
                <Text style={{ color: T.text, fontWeight: '600', fontSize: 14, textAlign: isRTL ? 'right' : 'left' }}>{user.email}</Text>
              </View>
            </View>
            ...
            <View style={{
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: T.subtext, fontSize: 12, marginBottom: 4, textAlign: isRTL ? 'right' : 'left' }}>{tr('phoneNumber')}</Text>
                <Text style={{ color: T.text, fontWeight: '600', fontSize: 14, textAlign: isRTL ? 'right' : 'left' }}>{user.phone}</Text>
              </View>
            </View>
