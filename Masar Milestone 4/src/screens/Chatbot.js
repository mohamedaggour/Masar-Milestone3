...
export default function Chatbot({ navigation }) {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: t(appLanguage, 'chatbotWelcome'),
      sender: 'bot',
      timestamp: new Date()
    }
  ]);
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const botText = data?.response || (data?.error ? `Error: ${data.error}` : tr('chatFallback'));
          ...
          {isLoading && (
            <View style={{ flexDirection: 'row', justifyContent: 'flex-start', marginBottom: 16 }}>
              ...
              <View style={{
                ...
                <Text style={{ color: T.subtext, fontSize: 14 }}>{tr('typing')}</Text>
              </View>
            </View>
          )}
        </ScrollView>
